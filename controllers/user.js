...
export const register = async (req, res) => {
  try {
    ...
    const hashpass = await bcrypt.hash(password, 10); // ✅ bcryptjs used here
    user = {
      name,
      email,
      password: hashpass,
    ...
    await sendMail(email, "EduCloud", data);
...
export const loginUser = TryCatch(async (req, res) => {
  ...
  const matchpassword = await bcrypt.compare(password, user.password); // ✅ bcryptjs used here
